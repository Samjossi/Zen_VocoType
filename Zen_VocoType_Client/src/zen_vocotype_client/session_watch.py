...
- QtDBus 信号投递依赖 Qt 事件循环（客户端由 ``QApplication.exec()`` 承载）；
- 回调必须轻量（仅投递退出事件），🔴 禁止阻塞/交互/弹窗。
"""
...
class SessionShutdownWatcher(QObject):
    ...
    :param on_shutdown: 关机触发回调（客户端为 ``QApplication.quit``，
        轻量投递，实际清理仍在事件循环内经 ``aboutToQuit`` 完成）。
    :param parent: Qt 父子关系宿主（随 ``QApplication`` 存活）。
    """
