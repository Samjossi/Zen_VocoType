...
2. 按住 `<ctrl>+<alt>+o` 说话，松开即识别并粘贴到当前光标处
3. 录音上限 60 秒，到达自动停止并识别（通知提示）
4. 粘贴完成后约 500ms 自动恢复你原来的剪贴板内容；若此间你复制了新内容，
   恢复自动放弃（不会覆盖你的新内容）

托盘右键菜单：`Zen_VocoType v0.2`（版本标识，版本号唯一真相为仓库根 versions.toml）/ 状态行 / 快捷键展示行 / 修改快捷键… / 重试连接服务端 / 退出。
...
3. 「确定」即换即用（不重启客户端）；「恢复默认」一键回到 `<ctrl>+<alt>+o`
...
| `hotkey` | `<ctrl>+<alt>+o` | pynput 组合键表达式；非法表达式启动即报错（退出码 2） |
