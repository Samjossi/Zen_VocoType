...
`5` 已有客户端实例运行；`6` 无显示环境（headless，T41）。
...
托盘右键菜单：`Zen_VocoType v0.2`（版本标识，版本号唯一真相为仓库根 versions.toml）/ 状态行 / 快捷键展示行 / 修改快捷键… / 剪贴板恢复延迟（XXXms）… / 保存录音（勾选项）/ 选择保存路径… / 打开保存文件夹 / 重试连接服务端 / 退出。
...
### 修改剪贴板恢复延迟（托盘菜单，即时生效 + 持久化）

1. 托盘右键 →「剪贴板恢复延迟（XXXms）…」（菜单文本即当前值），弹出数值输入框
2. 输入毫秒数（范围 0~10000，步进 50，预填当前值）→「确定」即改即用，
   下一次识别输出即按新延迟恢复剪贴板

- 调参指引：个别应用**粘贴出旧剪贴板内容**（读取偏慢）→ 调大（如 300~500）；
  想让原剪贴板更快恢复 → 调小（下限 0，立即恢复）。默认值 200
- 持久化位置与优先级链同快捷键：写入用户配置文件 `user_config.yaml`，
  环境变量 `ZEN_VOCOTYPE_CLIENT_PASTE_RESTORE_DELAY_MS` 优先级更高
  （切换成功时通知会如实提醒）
- 录音/识别进行中也可修改（该参数只影响下一次输出的调度，与录音状态无关）；
  落盘失败则本次修改整体不生效
...
| `paste_restore_delay_ms` | `200` | 粘贴后恢复原剪贴板的保守延迟（缩短剪贴板占用窗口；个别应用读取偏慢时可调大）；托盘「剪贴板恢复延迟…」可图形化修改并持久化 |
...
| 退出码 6 | 无显示环境（DISPLAY/WAYLAND_DISPLAY 均未设置，T41）：客户端托盘/热键/粘贴均依赖图形会话，拒绝启动（防 Qt 硬崩）。请在已登录的桌面会话内启动；SSH/systemd 等 headless 场景请仅运行服务端 |
