...
from zen_vocotype_service.inference.chunk_session import ChunkSessionRegistry
from zen_vocotype_service.state import ServiceState


class ServiceContext:
    """服务端运行时上下文（设置 / 状态 / 模型管理器 / 推理 worker / 会话表）。
    ...
    def __init__(self, settings: Settings, state: ServiceState) -> None:
        ...
        #: audio_chunk 全局会话表（v1.4；生命周期随 ctx，连接断开经
        #: ConnectionHandler 钩子按 owner 销毁其会话）
        self.chunk_sessions = ChunkSessionRegistry(settings)
