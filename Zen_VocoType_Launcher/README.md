...
| `auto_exit_delay_s` | 5 | T40：编排成功后托盘观察窗口秒数（0~60，0 = 立即退出；仅托盘模式有效；🔴 失败路径不自动退出） |
...
| 托盘「拉起来就消失」 | 非崩溃：`auto_exit_delay_s` 观察窗口结束自动退出（默认 5 秒）；失败时托盘会停留，对比可感知 |
